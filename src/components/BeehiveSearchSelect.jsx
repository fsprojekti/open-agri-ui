// file: src/components/BeehiveSearchSelect.jsx
...
/**
 * BeehiveSearchSelect (scrollable list with radio select, i18n-ready)
 *
 * Props:
 * - items: optional Array. If omitted, pulls from DbContext.beehives
 *          Each item can be { value, label } or any shape with getValue/getLabel.
 * - value: currently selected value (string, beehive UUID or code)
 * - onChange: (value) => void
 * - placeholder: string for the search input (default: t('beehive.searchPlaceholder'))
 * - label: string label above the control (default: t('beehive.select'))
 ...
 * - apiaryId: optional string; if present and items not provided, filters context beehives by apiary
 *
 * Behavior:
 ...
 */
export default function BeehiveSearchSelect({
                                                items,
                                                value,
                                                onChange,
                                                placeholder,
    ...
    // Expects `beehives` to contain entries with:
    //  - id or @id (UUID/IRI)
    //  - code or identifier / label fields
    //  - inApiary: uuid or relation (used for filtering by apiaryId if provided)
    const defaultItems = React.useMemo(() => {
        const list = Array.isArray(beehives) ? beehives : [];

        // optional: filter by apiaryId
        const matchUuid = (s) => String(s || "").match(/[0-9a-f-]{36}$/i)?.[0] || "";
        const filtered = apiaryId
            ? list.filter((h) => {
                const apiaryUuid =
                    matchUuid(h?.apiary?.["@id"] ?? h?.apiary?.id ?? h?.inApiary ?? "");
                return apiaryUuid === apiaryId;
            })
            : list;

        return filtered.map((h) => {
            const uuid = matchUuid(h?.["@id"] ?? h?.id ?? "");
            const code = h?.code || h?.identifier || `Hive ${uuid.slice(0, 8)}`;
